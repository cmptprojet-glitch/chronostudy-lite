...
import { StudySessionLog, SubjectMetric } from '../types';

/* ─────────────────────────────────────────────────────────────────────────────
   1. TABLEAU / GRAPHIQUE : ÉVOLUTION DU TEMPS D'ÉTUDE
   ───────────────────────────────────────────────────────────────────────────── */
export interface StudyTimeEvolutionProps {
  logs?: StudySessionLog[];
  className?: string;
  initialWeekOffset?: number;
}

export const StudyTimeEvolutionTable: React.FC<StudyTimeEvolutionProps> = ({
  logs = [],
  className = '',
  initialWeekOffset = 0,
}) => {
  ...
  const getWeekRangeLabel = (mondayDate: Date) => {
    ...
    const endStr = sundayDate.toLocaleDateString('fr-FR', {
      day: 'numeric',
      month: 'short',
      year: 'numeric',
    });
  ...
  const weeklyComparisonData = dayNames.map((name, index) => {
    ...
    return {
      day: dayCodes[index],
      fullName: name,
      fullDate: curDate.toLocaleDateString('fr-FR', {
        weekday: 'long',
        day: 'numeric',
        month: 'long',
      }),
      prevFullDate: prevDate.toLocaleDateString('fr-FR', {
        weekday: 'long',
        day: 'numeric',
        month: 'long',
      }),
  ...
  const overallWeekDiffHours = Number(
    ((currentWeekTotalMinutes - prevWeekTotalMinutes) / 60).toFixed(1)
  );
  ...
  return (
    <motion.div
      ...
      className={`bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800 p-6 rounded-3xl shadow-xs space-y-5 ${className}`}
    >
            ...
            {weekOffset === 0 && (
              <span
                style={{
                  ...
                  color: currentTheme.accentTextColor,
                }}
                  ...
                  return (
                    <div className="bg-[#161922] text-white p-3.5 rounded-2xl shadow-xl border border-zinc-700 text-xs space-y-2 min-w-[200px]">
                      ...
                      <div className="space-y-1">
                        <div className="flex justify-between text-zinc-300">
                          ...
                          <span className="font-black text-white">
                            {data.durationHours}h ({data.duration} min)
                          </span>
                        </div>
                        <div className="flex justify-between text-zinc-400">
                          <span>Même jour sem. -1 :</span>
                          <span className="font-bold">
                            {data.prevDurationHours}h ({data.prevDuration} min)
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="pt-1.5 border-t border-zinc-700 flex items-center justify-between">
                        ...
                        >
                          {isUp
                            ? `+${data.diffHours}h (+${data.percentDiff}%)`
                            : `${data.diffHours}h (${data.percentDiff}%)`}
                        </span>
                      </div>
                    </div>
      ...
      <div className="grid grid-cols-7 gap-1.5 pt-1">
        {weeklyComparisonData.map((d, idx) => {
          const isSelected =
            hoveredDayIndex === idx || (hoveredDayIndex === null && d.isToday);
...
/* ─────────────────────────────────────────────────────────────────────────────
   2. TABLEAU / GRAPHIQUE : RÉPARTITION PAR MATIÈRE
   ───────────────────────────────────────────────────────────────────────────── */
...
export const SubjectDistributionTable: React.FC<SubjectDistributionProps> = ({
  subjectMetrics = [],
  className = '',
}) => {
  const { currentTheme } = useTheme();
  ...
  return (
    <motion.div
      ...
      className={`bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800 p-6 rounded-3xl shadow-xs space-y-4 flex flex-col justify-between ${className}`}
    >
      <div>
                ...
                content={({ active, payload }) => {
                  if (active && payload && payload.length) {
                    ...
                    const pct = Math.round(
                      (item.hours / Number(totalSubjectHours || 1)) * 100
                    );
                    return (
                      <div className="bg-[#161922] text-white p-2.5 rounded-xl text-xs border border-zinc-700 shadow-xl">
                        <p style={{ color: currentTheme.accentColor }} className="font-black">
                          {item.subject}
                        </p>
                        <p className="text-zinc-300 font-bold">
                          {item.hours}h ({pct}% du total)
                        </p>
                      </div>
                    );
                  }
        ...
        {subjectMetrics.map((sub, idx) => {
          ...
          return (
            ...
            >
              ...
              <span className="font-black text-[#161922] dark:text-white shrink-0 ml-1">
                {sub.hours}h
              </span>
            </motion.div>
          );
...
/* ─────────────────────────────────────────────────────────────────────────────
   3. TABLEAU / GRAPHIQUE : ACTIVITÉ & COURBE D'ÉVOLUTION
   ───────────────────────────────────────────────────────────────────────────── */
export interface ActivityEvolutionProps {
  className?: string;
  initialTimeframe?: 'Weekly' | 'Monthly';
}

export const ActivityEvolutionCurveChart: React.FC<ActivityEvolutionProps> = ({
  className = '',
  initialTimeframe = 'Weekly',
}) => {
  const { currentTheme } = useTheme();
  const [activityTimeframe, setActivityTimeframe] = useState<'Weekly' | 'Monthly'>(
    initialTimeframe
  );
  ...
  const weeklyBarData = [
    ...
    { day: 'Mer', hours: 7.0, date: '16 Août', fullDate: 'Mercredi 16 Août' },
    {
      ...
      hours: 6.75,
      date: '17 Août',
      fullDate: "Jeudi 17 Août (Aujourd'hui)",
      isCurrent: true,
    },
  ...
  const activeHoverBarItem =
    hoveredBarIdx !== null && hoveredBarIdx < currentBarChartData.length
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true, margin: '-40px' }}
      transition={{ duration: 0.45, ease: [0.16, 1, 0.3, 1] }}
      className={`bg-white dark:bg-zinc-900 rounded-3xl p-6 shadow-xs border border-slate-100 dark:border-zinc-800 flex flex-col justify-between space-y-4 ${className}`}
    >
              ...
              >
                <div className="w-full bg-slate-100 dark:bg-zinc-800 h-28 rounded-full relative flex items-end justify-center p-0.5 overflow-hidden">
                  <motion.div
                    ...
                    style={{
                      backgroundColor:
                        item.isCurrent || isSelected ? currentTheme.accentColor : undefined,
                    }}
...
/* ─────────────────────────────────────────────────────────────────────────────
   4. TABLEAU / CARTE : MAÎTRISE DISCIPLINAIRE
   ───────────────────────────────────────────────────────────────────────────── */
export interface DisciplinaryMasteryProps {
  subjectMetrics: SubjectMetric[];
...
export const DisciplinaryMasteryTable: React.FC<DisciplinaryMasteryProps> = ({
  subjectMetrics = [],
  totalHours = '28.5',
  className = '',
  ...
  return (
    ...
    >
          ...
          >
            <div className="flex items-center justify-between text-xs font-bold">
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <span className="font-black text-[#161922] dark:text-white font-mono">
                  {metric.masteryPercentage}%
                </span>
              </div>
            </div>
      ...
      <div className="pt-2 border-t border-slate-100 dark:border-zinc-800 flex items-center justify-between text-[11px] text-slate-500 dark:text-slate-400">
        <span className="font-semibold">Objectif hebdomadaire : {targetWeeklyHours}h</span>
        <span className="font-black text-[#161922] dark:text-white">
          {totalHours}h / {targetWeeklyHours}h
        </span>
      </div>
    </motion.div>
  );
...
/* ─────────────────────────────────────────────────────────────────────────────
   5. COMPOSANT COMPOSITE : UI COMPONENTS (LES 4 TABLEAUX & COURBES RÉUNIS)
   ───────────────────────────────────────────────────────────────────────────── */
...
export const UIComponents: React.FC<UIComponentsProps> = ({
  logs = [],
  subjectMetrics = [],
  totalHours = '28.5',
  className = '',
  ...
  return (
    <div className={`space-y-5 ${className}`}>
      {/* RANGÉE 1 : ÉVOLUTION DU TEMPS D'ÉTUDE & RÉPARTITION PAR MATIÈRE */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        <div className="lg:col-span-7">
          <StudyTimeEvolutionTable logs={logs} />
        </div>
        <div className="lg:col-span-5">
          <SubjectDistributionTable subjectMetrics={subjectMetrics} />
        </div>
      </div>

      {/* RANGÉE 2 : ACTIVITÉ & COURBE D'ÉVOLUTION & MAÎTRISE DISCIPLINAIRE */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        <div className="lg:col-span-7">
          <ActivityEvolutionCurveChart />
        </div>
        <div className="lg:col-span-5">
          <DisciplinaryMasteryTable
